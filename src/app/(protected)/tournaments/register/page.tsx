...
/**
 * 💡 大会マップ: チーム参戦トラッカー v3.8 (Updated Path & Type-Safe)
 * 配置パス: src/app/(protected)/tournaments/map/page.tsx
 * * 🛠 更新事項:
 * 1. 登録画面への遷移先を /tournaments/register に更新。
 * 2. TypeScriptの厳格な型チェックを継続し、API連携の安全性を確保。
 * 3. 究極のUI/UX: 影を排除し、透過と繊細なラインで「プロの道具」を表現。
 */
import {
    Card,
    CardContent
...
function TournamentMapContent() {
    ...
    const analyzeTournamentPath = async () => {
        ...
        以下の3点を日本語のJSON形式で分析・回答してください。
        1. insight: 勝ち上がりのための戦略的な助言（100文字以内）
        2. mvp: 鍵を握る重要選手の名前
      ...
      `;
            ...
            const res = await fetch(apiUrl, {
                ...
                body: JSON.stringify({
                    ...
                    systemInstruction: {
                        parts: [{ text: "あなたはチーム専属の敏腕データアナリストです。監督、選手、ファンの士気が高まる、プロフェッショナルな分析を提供してください。" }]
                    }
                })
            });

            // 💡 明示的なキャストによる unknown 回避
            ...
            if (text) {
                setAiAnalysis(JSON.parse(text) as AiAnalysisResult);
                toast.success("最新の戦術分析を更新しました");
            }
        } catch (e) {
            console.error(e);
            toast.error("AI分析エンジンの接続に失敗しました");
        } finally {
            ...
            <div className="w-full bg-primary/5 border-b border-border/40 h-10 flex items-center overflow-hidden backdrop-blur-md">
                <div className="flex animate-[marquee_30s_linear_infinite] whitespace-nowrap gap-16">
                    <span className="text-[11px] font-bold text-primary flex items-center gap-2">
                        <Flame className="h-3.5 w-3.5" /> 速報：準々決勝の相手が「ベアーズ」に決定
                    </span>
                    <span className="text-[11px] font-bold text-muted-foreground flex items-center gap-2">
                        お知らせ：春季大会の組合せ表が更新されました
                    </span>
                    <span className="text-[11px] font-bold text-primary flex items-center gap-2">
                        <Zap className="h-3.5 w-3.5" /> AI分析：現在の優勝確率は前回比で微増しています
                    </span>
                </div>
            </div>
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-10 animate-in fade-in duration-700">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border/40 pb-8">
                    ...
                    <div className="flex items-center gap-3">
                        <Button
                            variant="outline"
                            // 💡 パス修正: tournaments ➔ tournaments/register
                            onClick={() => navigateTo('/tournaments/register')}
                            className="rounded-2xl h-12 px-6 border-border bg-card/40 backdrop-blur-md hover:bg-muted font-bold text-xs transition-all active:scale-95"
                        >
                            大会の新規登録
                        </Button>
                    </div>
                        ...
                        <CardContent className="p-6 sm:p-10">
                            {!aiAnalysis ? (
                                <div className="flex flex-col md:flex-row items-center gap-8">
                                    ...
                                    <div className="flex-1 text-center md:text-left space-y-1">
                                        ...
                                        <p className="text-xs font-bold text-muted-foreground leading-relaxed">
                                            参戦中のデータを元に、次戦の勝利戦略とチームの勝率を算出します。
                                        </p>
                                        <div className="pt-2">
                                            <span className="inline-flex items-center gap-1.5 text-primary font-black text-[9px] uppercase tracking-[0.1em] bg-primary/10 px-3 py-1.5 rounded-full">
                                                解析を実行する <ChevronRight className="h-3 w-3" />
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="animate-in zoom-in-95 duration-500 space-y-6">
                                    <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                                        <div className="space-y-4 flex-1">
                                            <div className="flex items-center gap-2">
                                                <Badge className="bg-primary text-primary-foreground border-none font-black text-[9px] uppercase px-3 rounded-md">Analysis Update</Badge>
                                            ...
                                            <div className="flex items-center gap-3 text-xs font-bold text-muted-foreground pl-5">
                                                ...
                                                次戦のキーマン: <span className="text-foreground border-b border-primary/20 font-black">{aiAnalysis.mvp}</span>
                                            </div>
                                        </div>
                                        {/* 確率サークル */}
                                    ...
                                    <Button variant="ghost" onClick={() => setAiAnalysis(null)} className="w-full border-t border-border/40 rounded-none h-10 text-[10px] font-black uppercase tracking-widest hover:bg-primary/5">シミュレーターをリセット</Button>
                                </div>
                            )}
                        </CardContent>
                    </Card>
                ...
                <div className="pt-12 pb-10 flex flex-col items-center gap-5">
                    ...
                    <div className="flex items-center gap-3 px-8 py-2.5 rounded-full bg-card/20 border border-border/40 backdrop-blur-md">
                        ...
                        <p className="text-[10px] font-black italic text-muted-foreground tracking-[0.4em] uppercase text-center">
                            Master the journey, own the victory
                        </p>
                    </div>
                </div>
