...
import { authClient } from "@/lib/auth-client"; // 🌟 セッション取得用
...
export default function DashboardPage() {
  ...
  const [matches, setMatches] = useState<Match[]>([]); // any[] から Match[] へ
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (matchRes.ok) {
          ...
          const sorted = Array.isArray(matchData)
            ? matchData.sort((a, b) => b.date.localeCompare(a.date))
            : [];
          setMatches(sorted);
        }
      } catch (error) {
