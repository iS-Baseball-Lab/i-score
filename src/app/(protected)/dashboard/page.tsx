  ...
  const handleCreateTeam = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetchTeams();
      } else toast.error("チームの作成に失敗しました");
    } catch (e) { console.error(e); }
    ...
    return (
      <div className="container mx-auto max-w-xl px-4 py-12 animate-in fade-in duration-500">
        <div className="text-center mb-8">
          <div className="h-24 w-24 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner border border-primary/20">
            <Users className="h-12 w-12 text-primary" />
          </div>
          <h1 className="text-3xl font-black tracking-tight mb-2">ようこそ i-Score へ！</h1>
          <p className="text-muted-foreground font-bold">まずはあなたのチームを作成するか、招待をお待ちください。</p>
        </div>

        <Card className="rounded-[32px] border-border/50 shadow-sm overflow-hidden">
          <CardHeader className="bg-muted/30 pb-4 border-b border-border/50">
            <CardTitle className="flex items-center gap-2 font-black text-xl"><Plus className="h-6 w-6 text-primary" /> チームを新規作成</CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <form onSubmit={handleCreateTeam} className="space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-extrabold uppercase tracking-wider text-muted-foreground">チーム名</label>
                <input type="text" required placeholder="例: 川崎中央シニア" className="flex h-14 w-full rounded-2xl border border-border/60 bg-muted/20 px-4 text-base font-bold shadow-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" value={newTeamName} onChange={(e) => setNewTeamName(e.target.value)} />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-extrabold uppercase tracking-wider text-muted-foreground">あなたの役割（ロール）</label>
                <Select className="flex h-14 w-full rounded-2xl border border-border/60 bg-muted/20 px-4 text-base font-bold shadow-xs focus-visible:ring-primary" value={newTeamRole} onChange={(e) => setNewTeamRole(e.target.value)}>
              ...
              <Button type="submit" className="w-full h-14 text-base font-extrabold rounded-2xl mt-6 shadow-md hover:-translate-y-1 transition-all" disabled={isCreating}>
                {isCreating ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : "チームを作成して始める"}
              </Button>
            </form>
          </CardContent>
        </Card>
            ...
            <Card className="relative overflow-hidden group rounded-[32px] border-primary/30 bg-primary/5 shadow-xs transition-all duration-300 hover:shadow-md hover:border-primary/50 hover:-translate-y-1 cursor-pointer h-full">
              <div className="relative z-10 flex flex-col h-full justify-between">
                <CardHeader className="pb-4">
                  ...
                  <CardDescription className="text-sm font-bold mt-2">スコアブックの入力を開始します</CardDescription>
                </CardHeader>
                <CardContent>
