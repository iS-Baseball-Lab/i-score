...
import { matches, atBats, pitches, playLogs, matchLineups } from '@/db/schema' // 💡 matchLineupsを追加
...
app.post('/', async (c) => {
    ...
    try {
        await db.insert(matches).values({
            ...
            surfaceDetails: body.location || null, // 旧locationは一旦こちらに退避
    ...
    try {
        ...
        if (!currentAtBat) {
            ...
            await db.insert(atBats).values({
                ...
                batterId: body.batterId || null, // NameからIdに変更
        ...
        await db.insert(pitches).values({
            ...
            zoneX: body.zoneX ?? null, zoneY: body.zoneY ?? null,
        })
...
app.post("/:id/logs", async (c) => {
    ...
    try {
        ...
        await db.insert(playLogs).values({
            ...
            // ⚠️ batterName と timestamp は新スキーマから消えたため除外
        }).run();
...
app.delete("/:id/logs/:logId", async (c) => {
    ...
    const db = drizzle(c.env.DB);
    try {
        await db.delete(playLogs).where(and(eq(playLogs.id, logId), eq(playLogs.matchId, matchId))).run();
